...
def brain_init():
    """create the board and call pipe_out("OK") or pipe_out("ERROR Maximal board size is ..")"""
    raise NotImplementedError


def brain_restart():
    """delete old board, create new board, call pipe_out("OK")"""
    raise NotImplementedError
...
def parse_3int_chk(param):
    ...
    x, y, z = param.split(',')
    x, y, z = [to_int(v) for v in (x, y, z)]
    if any(v is None for v in (x, y, z)):
        return None, None, None
    return x, y, z


def get_cmd_param(command, value):
...
def do_command(cmd):
    """do command cmd"""
    global info_max_memory, info_timeout_match, info_timeout_turn, info_time_left, \
        info_game_type, info_exact5, info_continuous, info_renju, dataFolder
    global width, height
    #
    ...
    if param is not None:
        if param.count(',') != 1:
            ...
            width, height = [to_int(v) for v in (width, height)]
