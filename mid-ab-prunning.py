...
class game:
    ...
    def brain_init(self):
        if pp.width < 5 or pp.height < 5:
            ...
            return
        if pp.width > self.MAX_BOARD or pp.height > self.MAX_BOARD:
            pp.pipeOut("ERROR Maximal board size is {}".format(self.MAX_BOARD))
            return
        pp.pipeOut("OK")
    ...
    def brain_about(self):
        pp.pipeOut(pp.infotext)

    def brain_eval(self, x, y):
...
pp.brain_init = A.brain_init
pp.brain_restart = A.brain_restart
pp.brain_my = A.brain_my
pp.brain_opponents = A.brain_opponents
pp.brain_block = A.brain_block
pp.brain_takeback = A.brain_takeback
pp.brain_turn = A.brain_turn
pp.brain_end = A.brain_end
pp.brain_about = A.brain_about
if DEBUG_EVAL:
    pp.brain_eval = A.brain_eval
